...
public class FurnitureDeliveryRequestDAO implements IDao<FurnitureDeliveryRequest> {
    public List<FurnitureDeliveryRequest> fetchAllObjects() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                FurnitureDeliveryRequest request = new FurnitureDeliveryRequest(requestID, new Requester(requestID, requester), furnitureType, additionalNotes, deliveryTime, deliveryLocation);
    ...
    public FurnitureDeliveryRequest addRow(FurnitureDeliveryRequest orm){
        ...
        try {
            String query = "INSERT INTO \"ServiceRequest\".\"furnitureDeliveryRequest\" (requestID, Requester, furnitureType, additionalNotes, ETA, roomName) VALUES (?,?,?,?,?,?)";
    ...
    public FurnitureDeliveryRequest updateRow(FurnitureDeliveryRequest orm, FurnitureDeliveryRequest orm2){
        DBConnection db = new DBConnection();
        try {
            String query = "UPDATE \"ServiceRequest\".\"furnitureDeliveryRequest\" SET Requester = ?, furnitureType = ?, additionalNotes = ?, ETA = ?, roomName = ? WHERE requestID = ?";
            PreparedStatement ps = db.getConnection().prepareStatement(query);
